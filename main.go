...
func transcriptionHandler(w http.ResponseWriter, r *http.Request) {
    ...
    // Verifica se os dados decodificados estão vazios
    if len(data) == 0 {
        resp := TranscriptionResponse{Error: "Dados decodificados estão vazios"}
        w.Header().Set("Content-Type", "application/json")
        json.NewEncoder(w).Encode(resp)
        return
    }
    log.Printf("Tamanho dos dados decodificados: %d bytes", len(data))

    // Salva o arquivo de entrada
    var inputFile, outputFile string
    switch req.MediaType {
    case "audio":
        inputFile = filepath.Join(tempDir, "input")
    ...
    case "video":
        ...
        start := time.Now()
        ffmpegArgs := []string{"-i", inputFile, "-vn", "-ar", "16000", "-ac", "1", "-af", "afftdn", "-y", outputFile}
        cmd := exec.Command("ffmpeg", ffmpegArgs...)
        var stderr strings.Builder
        cmd.Stderr = &stderr
        err = cmd.Run()
    ...
    // Gera áudio de resposta com piper-tts
